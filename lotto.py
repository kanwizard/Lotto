...
def get_recent_lotto_numbers(latest_draw):
    return [get_lotto_numbers_by_draw(latest_draw - i) for i in range(3)]
...
def main():
    ...
    <p style="font-size: 18px;">
        입력한 회차부터 이전 3회차 까지의 당첨 번호를 분석하여 생성함
    </p>
    ...
    if generate_button:
        if latest_draw > 0 and num_combinations > 0:
            # 최신 회차 번호를 가져옵니다.
            recent_numbers = get_recent_lotto_numbers(latest_draw)  # 최근 3회차 번호 가져오기
            
            # 최근 3회차 당첨 번호 출력
            st.subheader("📅 최근 3회차 당첨 번호")
